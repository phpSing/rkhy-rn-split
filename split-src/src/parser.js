...
class Parser {
  ...
  splitBundle() {
    ...
    this._bundles.forEach(subBundle => {
      ...
      console.log('[Code] 写入代码 ' + codePath);
      ...
      console.log('====== 分离 ' + subBundle.name + ' 成功! ======');
    });
  }
  ...
  _parseAST(bundleAST : any) {
    ...
    body.forEach(node => {
      ...
      if (Util.isPolyfillCall(node, this._config.dev)) { // push polyfill codes to base.
        ...
        if (Util.isAssetModule(moduleName)) {
          ...
          console.log('获取 asset 模块 ' + moduleName, module.assetConfig);
        }
        ...
        if (this._isBaseEntryModule(module)) {
          ...
          console.log('获取自定义base模块入口 ' + moduleName);
        ...
        // console.log('模块 ' + moduleName + '(' + moduleId + ') dependency:' + JSON.stringify(module.dependencies));
      } else {
        console.log(require('util').inspect(node, false, null));
        console.log('无法解析节点!', this._codeBlob.substring(node.start, node.end));
      }
    });
    ...
    console.log('after gen base module, generate custom entry sets.', customEntry, !!customEntry)
    if (!!customEntry) {
  ...
  _getAssetRenames(asset : Asset,
    ...
    const assetRenames = [];
    console.log('this._config.platform', this._config.platform)
  ...
  _doSplit() {
    ...
    if (this._useCustomSplit) {
      console.log('this._customEntries', this._customEntries)
      this._customEntries.forEach(entry => {
        console.log('分离 custome ....entry', entry)
